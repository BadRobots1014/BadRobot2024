...
public class ShootAndDriveRightAutoCommand extends SequentialCommandGroup {
  @SuppressWarnings({"PMD.UnusedPrivateField", "PMD.SingularField"})

  public ShootAndDriveRightAutoCommand(ShooterSubsystem shoot, SwerveSubsystem swerve) {
    super(
      new ShootCommand(shoot).withTimeout(4),
      new SwerveDriveCommand(swerve, supplyDouble(-.3), supplyDouble(-.3), supplyDouble(0), supplyBoolean(false), supplyBoolean(false))
      .withTimeout(3)
    );
  }
