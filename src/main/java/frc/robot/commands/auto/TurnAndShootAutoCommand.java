...
public class TurnAndShootAutoCommand extends SequentialCommandGroup {
  public TurnAndShootAutoCommand(LimelightSubsystem lime, ShooterSubsystem shoot, SwerveSubsystem swerve, IntakeSubsystem intake, Pose2d startingOffset, double turnDegrees, double delay) {
    super(
      new WaitCommand(delay),
      new SetPoseCommand(swerve, startingOffset).withTimeout(0),
      new SwerveDriveCommand(swerve, lime, shoot,supplyDouble(0), supplyDouble(-.3), supplyDouble(0), true, supplyBoolean(true), supplyBoolean(false), supplyDouble(-1), supplyInteger(DriveConstants.kAutoAimInactive))
      .withTimeout(1.9),
